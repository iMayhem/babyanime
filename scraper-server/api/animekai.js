// AnimeKai Scraper for Nuvio / BabyAnime Local Scrapers
// Uses enc-dec.app database for accurate matching
...
// Search AniList by AniList ID directly if available
function getAniListById(anilistId) {
    var query = 'query ($id: Int) { Media(id: $id, type: ANIME) { id idMal title { english romaji native } startDate { year } } }';
    return fetchRequest(ANILIST_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({ query: query, variables: { id: parseInt(anilistId) } })
    })
        .then(function (res) { return res.json(); })
...
function getStreams(tmdbId, mediaType, season, episode) {
    // Standardize mediaType (default 'tv' if not provided)
    if (!mediaType) mediaType = 'tv';

    var rid = createRequestId();
    logRid(rid, 'getStreams start', { tmdbId: tmdbId, mediaType: mediaType, season: season, episode: episode });

    var mediaInfo = { title: 'Anime', year: null };
    var dbResult = null;

    // Check if tmdbId is passed as 'anilist:185874' or raw ID
    var anilistIdDirect = null;
    if (typeof tmdbId === 'string' && tmdbId.startsWith('anilist:')) {
        anilistIdDirect = tmdbId.replace('anilist:', '');
    }

    var fetchAniListPromise = null;
    if (anilistIdDirect) {
        fetchAniListPromise = getAniListById(anilistIdDirect);
    } else {
        // Step 1: Get anime title from TMDB
        fetchAniListPromise = getTMDBDetails(tmdbId)
    ...
    return fetchAniListPromise
        .then(function (anilistData) {
            if (!anilistData || !anilistData.malId) {
                throw new Error('Could not find MAL ID from AniList');
            }
            if (anilistData.title && (anilistData.title.english || anilistData.title.romaji)) {
                mediaInfo.title = anilistData.title.english || anilistData.title.romaji;
            }
            if (anilistData.year) mediaInfo.year = anilistData.year;
        ...
        .then(function (result) {
            ...
            if (episodes && episodes[selectedSeason] && episodes[selectedSeason][selectedEpisode]) {
                ...
                // Fallback: try episode count matching across seasons
                var seasons = Object.keys(episodes || {});
                if (seasons.length > 0) {
                    var firstSeason = seasons[0];
                    if (episodes[firstSeason] && episodes[firstSeason][selectedEpisode]) {
                        token = episodes[firstSeason][selectedEpisode].token;
                        logRid(rid, 'fallback: found E' + selectedEpisode + ' in S' + firstSeason);
                    } else {

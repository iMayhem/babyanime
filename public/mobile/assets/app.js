...
(function() {
  ...
  window.toggleBookmark = function(animeData) {
    const result = _origToggle ? _origToggle(animeData) : (() => { return false; })();
    if (_currentUser) {
      const idStr = String(animeData.id);
      const type = animeData.type || 'anilist';
      const sb = sbClient();
...
function createAnimeCardHTML(anime) {
  ...
  const cardImgId = 'img_' + Math.random().toString(36).substring(2, 9);
...
function setupHoldToSwap() {
  ...
  function getSwappableItem(target) {
    ...
    if (target.closest('input, select, textarea, .clr-dot, .schedule-tab-btn, .ep-btn')) return null;

    return target.closest('.card, .anime-card, .shelf-item, .pill-opt, .genre-tag, .room-card, .airing-card, .player-column, .sidebar-column, .airing-sidebar, .details-box, .episodes-panel, .selector-section, .tab-btn, .tt, .player-wrapper, .player-card, #playerContainer');
  }
